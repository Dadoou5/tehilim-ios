...
struct Prayer: Identifiable, Hashable {
    enum Kind: String, Identifiable, Hashable, CaseIterable {
        ...
        var titleFR: String {
            switch self {
            case .before: return L("Prière avant la lecture")
            case .after:  return L("Prière après la lecture")
            }
        }

        var subtitleFR: String {
            switch self {
            case .before: return L("Tehilim 95, versets 1 à 3")
            case .after:  return L("Tehilim 14:7 et 37, versets 39 à 40")
            }
        }
