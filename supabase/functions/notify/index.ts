...
function messageFor(event: string, value: number | null, chainName: string, locale: string) {
  ...
  const lang = l.startsWith("en") ? "en" : (l.startsWith("he") || l.startsWith("iw")) ? "he" : "fr";
  const pick = (fr: { title: string; body: string }, en: { title: string; body: string }, he: { title: string; body: string }) =>
